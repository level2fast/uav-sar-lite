...
% multiple of the number of lines per block.Because the data on CD has a 
% pulse replica stored every 8 range lines, it is convenient to
% read the CD 8 lines at a time. To simplify the read procedure, the first 
% range line and number of range lines per block are restricted to be 
% multiples of 8.  If these rules are not obeyed, the "extract_data.m" 
% program will quantize the values accordingly.
...
% Nrg_lines     : number of range lines to extract(must be a multiple of 
%                 the number of lines per block)
% Nrg_lines_blk : number of range lines per block(allows you to control the 
%                 extracted file size, so that the data can be extracted into a number of 
%                 smaller files, rather than one big file, should be set to a multiple of 8)
...
data_output_path = fileparts(mfilename('fullpath')) + "\data\";
% Define location of data files
input_path        = 'H:\UAVSARLITE_APP_Reference\Digital Signal Processing of Synthetic Aperture Radar Data\Cumming_058-3\scene01\';
...
Nrg_cells       =  2048;    % Suggest 2048 cells(number range cells/samples that will be taken from  data)
Nrg_lines_blk   =  6*256;   % Suggest 1536, it should be larger than the
                            ...
                            % (This is the number of range lines(azimuth)
                            % that make up 1 block of data. A block being
                            % what we define it as. The radar data can be 
                            % written in one block, or broken up into a 
                            % number of blocks, with a set of files for 
                            % each block.
Nrg_lines   =  2*Nrg_lines_blk; %Extract 2 blocks worth of data 
...
%  Save the data and run parameters in a matlab data file (*.mat) for use 
%  by the subsequent data extraction and processing programs.
vv = version;   vers = str2num(vv(1));  % Find MATLAB version
if vers == 6,   SaveV6 = 0;   end
addpath(data_output_path)  
if SaveV6
  save -v6 data/CD_run_params
  fprintf('\nData parameters written to:  CD_run_params.mat  in V6 format')
else
  save(data_output_path + "CD_run_params.mat")
