...
% Nrg_cells      : Number of samples in range direction
...
num_pixel_data   = 2*tot_Nrg_cells;                               %  18576(multiplication done to account for real and img parts of range cells.Each pixel has a real an imaginary part)
h                = num_header;                                    %    192
...
rep_block_length = length_replica + 8*hpa ; % (2880 bytes + 8 * 18818 )

start_rep        = (first_replica-1)*hpa + num_header + num_aux + 1; 
end_rep          = start_rep + length_replica - 1 ;

%multiply first range cell by 2 to account for real and imaginary part of
%pixel data. Each pixel has real and imaginary part so the first range cell
%is actually 2 times to the right of it's normal location
start_col        = ha + 2*(first_rg_cell - 1) + 1 ; % must account for header and auxiliary number
end_col          = start_col + 2*Nrg_cells - 1;

% The number of bytes to skip from the beginning of the file (Nbytes_skip)
Nrep_blks_row1   = ceil(start_line_blk/8) - 1; % this values is the number of bytes we need to skip to get to the correc range line

Nbytes_blocks    = Nrep_blks_row1 * rep_block_length;  % this value describes the total number of bytes in 2D block(think bytes(az) * bytes(rng))
                                                       % that we need to
                                                       % skip to get to the
                                                       % the range line we
                                                       % want to start with

Nbytes_skip      = file_header_length + Nbytes_blocks; % this values describes te total number of bytes 
                                                       % we need to skip by including the unused header data
...
for kb = 1 : N_8line_blocks   %  Read and decode 8 lines at a time
    ...
    % read one 8-line 2D block of radar data and replica
    temp = uint8( fread( fid, [rep_block_length,1], 'uint8' ) );
    
    % separate the replica and 8 lines of [header, aux and data]
    replica = temp(start_rep : end_rep)'; % index temp(bytes(start_rep):bytes(end_rep))'
    temp = [ temp(1 : start_rep-1)' temp(end_rep+1 : rep_block_length)' ];
    temp = reshape(temp, hpa, 8)';   % Shape array as 8 rows by all columns where each row
                                     % is left to right: header, aux, and pixel # bytes

    % Extract the desired set of range cells by skipping over header & aux
    % bytes and just including columns # of bytes specified. Store 8 rows
    % of data at a time. 
...
if UseMATfiles == 1
    ...
    if vers == 7
        ...
        % convert data to double precision and remove the upper 16 bits
        % then convert to an 8-bit signed integer
    ...
    if SaveV6
       ...
       save(strcat(output_path, 'CDdata', num2str(blk)), 'data')
