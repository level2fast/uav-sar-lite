...
function [CompressedSignal] = rng_compress_signal(LinearFMChirp,NoisySignal,peformIFFT,NumberOfSamples)
...
% FFT on each range cell(row) of data. See FFT(X,[],DIM) or fft(X,n,2) in
...
if(peformIFFT)
    ...
    CompressedSignal(:,size(LinearFMChirp,2)+1:end) = []; % throwaway region
